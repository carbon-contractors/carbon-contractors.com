...
## 28. I verified the hazard I had been warned about and not the one beside it

**2026-08-16, resolving a merge conflict.**

`CLAUDE.md` carries a warning about text files damaged by Windows tooling: *"Line endings are
pinned. Python's `write_text` silently writes CRLF on Windows — use `write_bytes`."*

Resolving PR #95's conflicts, I rewrote three files through PowerShell. `Get-Content` without
`-Encoding` reads a BOM-less file as the system ANSI codepage on Windows PowerShell 5.1, so every
multi-byte character was decoded as CP-1252 and then written back out as UTF-8. `—` (`E2 80 94`)
became `â€"`. 248 lines across `CLAUDE.md`, `docs/Lessons-Learned.md` and `docs/backlog/CC-025.md`.

**I checked. I ran `git ls-files --eol`, got `i/lf w/lf` on all five files, and was satisfied.**
That is the exact hazard the file warns about, verified on the exact tool that had just caused a
different one. Encoding and line endings are damaged by the same operation, by the same tool, in the
same commit — and I confirmed one and never thought about the other.

### Why every existing control missed it

- **The output is still valid UTF-8.** It is not malformed; it is correctly-encoded nonsense. No
  parser, linter or editor objects. `git` has no opinion — `.gitattributes` pins *line endings*,
  and `text=auto eol=lf` says nothing about character encoding.
- **`git ls-files --eol` reports `lf/lf` on a thoroughly corrupted file**, which is worse than no
  signal: it is a green light on an adjacent question.
- **The corrupting tool is the displaying tool.** PowerShell renders *clean* UTF-8 as mojibake too,
  so healthy and damaged files look identical in the terminal. I printed `â€"` to the console twice
  while inspecting issue titles and read past it both times — correctly, in general, which is the
  problem.
- **Review does not catch it.** The diff is 268 insertions and 268 deletions of visually similar
  lines, in a merge commit, in documentation.

### What made it recoverable

Double-encoding is losslessly reversible, and provably so: re-encode the suspect run as CP-1252,
decode as UTF-8, and require that re-encoding the result reproduces the original bytes exactly. A
genuine standalone `é` is byte `E9`, which is not valid UTF-8 alone, so it fails that test and is
left untouched. That asymmetry is what makes a whole-tree repair safe rather than a gamble.

A whole-*file* transcode, by contrast, fails — the repaired lines contain `→`, which CP-1252 cannot
represent. The unit of repair has to be the run, not the file.

Two independent repairs — one by the session that found it, one written from the corrupted blob
without seeing the first — produced byte-identical output. That agreement is the evidence the fix
was right, not either implementation on its own.

### The generalisable form

**A warning about a hazard is not coverage of the class it belongs to.** `CLAUDE.md` named CRLF, so
CRLF was checked; encoding, the same tool's other failure mode, was not on the list and therefore
was not on my mind. A documented hazard narrows attention as much as it directs it.

The durable fix is not another line in `CLAUDE.md`. It is
`scripts/audit/check-encoding.mjs`, offline and wired into CI beside the workflow linter and the
