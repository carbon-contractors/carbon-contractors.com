...
If you want the short version: **we collect the minimum needed to run a worker directory, we don't
run analytics or trackers of any kind, we don't sell or share your data with anyone, and you can ask
us to delete anything we hold on you at any time.**

There is no waitlist and there never will be one — the platform holds no email addresses of people
who are not workers. The only optional email the platform can hold is a contact channel a registered
worker chooses to supply (see below), and it exists solely so we can tell you that you've been
hired.
...
notified when you're hired. No contact channel is ever required, and none is verified — supplying
one proves nothing about who you are. It exists solely to notify you, is never handed to the agent
that hires you, and is never public. Delete it at any time and the platform holds no way to reach
you off-chain.
...
Your contact channel (email, webhook, etc.) and your task history's descriptions are **not** part
of that public listing.

## Where your data lives

Worker registrations and task records are stored in a managed Postgres
...
Everything described elsewhere on this page (your optional contact channel, your task descriptions)
lives in our own database and can be deleted on request. On-chain transaction history cannot.
...
- delete your worker registration or contact channel.
